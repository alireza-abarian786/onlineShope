...
// import { addToCartAndToggleButton } from "./box.js";
//!---------------------------------------------------------------------- functions -------------------------------------------------------
...
let changeBtnAfterDelete = async (element) => {
  document.querySelectorAll(".product-box").forEach(async (box) => {
    ...
    if (titleCart === titleBox) {
      //* اگر عنوان محصول و عنوان کارت یکی بود
      if (card.querySelector(".add-cart p")) {
        //* حالت ستونی
        card
          .querySelector(".btn-cart-box")
          .classList.remove("add-cart-active-btn"); //* اعمال کلاس جدید کلید سبد خرید
        card.querySelector(".add-cart > p").textContent = "اضافه به سبد خرید"; //* عنوان کلید سبد خرید
        card
          .querySelector(".add-cart > p")
          .classList.remove("add-cart-active-content"); //* اعمال کلاس جدید به عنوان کلید سبد خرید
        card
...
//     console.warn("⚠️ No products to display for this arrAppliances.");
//     return;
//   }
...
//                       تومان
//                       <span class="ms-1">${(box.price - (box.price * (Math.floor(box.discount / 10000) / 100))).toLocaleString()}</span>
//                       &nbsp&nbsp:قیمت با تخفیف
//                     </span>`
//                 }
//               </div>
//           </div>
//           <div class="add-cart btn-cart-box" type="button" id="liveToastBtn" onclick="addToCartAndToggleButton(event , ${box._id})">
...
let createBox = (arrCategory) => {
  ...
  if (arrCategory.length) {
    //? اگر محصولی وجود داشت، نمایش بده
    arrCategory.forEach((product) => {
      document.querySelector(".cantainer-category__footer").insertAdjacentHTML(
        "beforeend",
        `
    ...
    //? اگر محصولی پیدا نشد، پیام خطا نمایش بده
    document
      .querySelector(".cantainer-category__footer")
      .insertAdjacentHTML(
        "beforeend",
