...
import { handleAddToCart } from "./funcs/store/box.js";
import { initializeStatus } from "./funcs/store/bookMarks.js";
import { toggleBookmark } from "./funcs/store/bookMarks.js";
...
let createBoxCategory = (data) => {
  ...
  createBox(arrCategory)
}

let showSearchProducts = async (data , url) => {
  boxSerchInput.addEventListener('input', (e) => {
    let showProduct = getSearchProduct(data[url] , 'title' , e.target.value)
    createBox(showProduct)    
  })
}
